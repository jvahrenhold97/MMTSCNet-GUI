...
def get_trained_model_folder(modeldir, capsel, growsel, start_btn, progbar, output_log):
    """
    Retrieves the filepath for the most recently created model instance.
...
def predict_for_data(pretrained_model, X_pc, X_metrics, X_img_1, X_img_2, onehot_to_label_dict, filtered_pointclouds, las_unzipped_path, model_dir, capsel, growsel, output_log):
    """
    ...
    for i in range(0, len(filtered_pointclouds)):
        ...
        output_log.insert("1.0", f"{now_formatted} - STATUS - Label for point cloud {filtered_pointclouds[i]}: {current_label}!" +'\n')
            ...
            if subfolder == current_plot:
                ...
                for folder in os.listdir(subfolder_path):
                    if folder == "ULS":
                        ...
                        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
                        output_log.configure(state="normal")
                        output_log.insert("1.0", f"{now_formatted} - STATUS - Building KDTree!" +'\n')
                        output_log.configure(state="disabled")
                        ...
                        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
                        output_log.configure(state="normal")
                        output_log.insert("1.0", f"{now_formatted} - STATUS - Querying source point cloud!" +'\n')
                        output_log.configure(state="disabled")
                        ...
                        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
                        output_log.configure(state="normal")
                        output_log.insert("1.0", f"{now_formatted} - STATUS - Wrote predicted label {current_label} to source point cloud!" +'\n')
                        output_log.configure(state="disabled")
