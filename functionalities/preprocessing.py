...
def augment_species_pointclouds(species_pcs, max_representation, species_distribution, max_scale, pc_path_selection, output_log):
    pc_index = 0
    for pointcloud in species_pcs:
        ...
        for i in range(0, int(upscale_fac)*4):
            ...
            output_log.insert("1.0", f"{now_formatted} - STATUS - Created a new point cloud!" +'\n')
...
def augment_selection_fwf(pointclouds, fwf_pointclouds, elimination_percentage, max_pc_scale, pc_path_selection, fwf_path_selection, pc_size, capsel, output_log):
    if check_if_data_is_augmented_already(pointclouds) == False:
        ...
        augment_species_pointclouds_fwf(species_pc_pairs, max_representation, species_distribution, max_pc_scale, pc_path_selection, fwf_path_selection, output_log)
    else:
        now = datetime.datetime.now()
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - WARNING - Previously augmented data found, loading!" +'\n')
        output_log.configure(state="disabled")

def augment_selection(pointclouds, elimination_percentage, max_pc_scale, pc_path_selection, pc_size, capsel, output_log):
    if check_if_data_is_augmented_already(pointclouds) == False:
        ...
        max_representation = get_maximum_distribution(species_distribution)
        augment_species_pointclouds(species_pointclouds, max_representation, species_distribution, max_pc_scale, pc_path_selection, output_log)
    else:
        now = datetime.datetime.now()
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
...
def generate_colored_images(IMG_SIZE, las_working_folder, img_working_folder, output_log):
    if get_colored_images_generated(las_working_folder, img_working_folder) == False:
        ...
        for pointcloud in os.listdir(las_working_folder):
            ...
            now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
            output_log.configure(state="normal")
            output_log.insert("1.0", f"{now_formatted} - STATUS - Generated frontal and sideways ciew of point cloud {pcid}!" +'\n')
            output_log.configure(state="disabled")
    else:
        pass
...
def resample_point_cloud_density_based(points, num_points, output_log, num_segments=20):
    now = datetime.datetime.now()
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - STATUS - Resampled a point cloud from {len(points)} to {num_points} points!" +'\n')
    output_log.configure(state="disabled")
...
def generate_metrics_for_selected_pointclouds_fwf(selected_pointclouds, filtered_fwf_pointclouds, metrics_dir, capsel, growsel, output_log):
    ...
    if workspace_setup.get_are_fwf_pcs_extracted(metrics_dir) == False:
        ...
        for i in range(len(selected_pointclouds)):
            ...
            now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
            output_log.configure(state="normal")
            output_log.insert("1.0", f"{now_formatted} - STATUS - Computed {i+1}/{len(selected_pointclouds)} metrics!" +'\n')
            output_log.configure(state="disabled")
        ...
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - WARNING - Previously generated metrics found, importing!" +'\n')
        output_log.configure(state="disabled")
        combined_metrics = load_metrics_from_path(metrics_path)
    return combined_metrics

def generate_metrics_for_selected_pointclouds(selected_pointclouds, metrics_dir, capsel, growsel, output_log):
    savename = f"training_generated_metrics_{capsel}_{growsel}.csv"
    metrics_path = main_utils.join_paths(metrics_dir, savename)
    if workspace_setup.get_are_fwf_pcs_extracted(metrics_dir) == False:
        all_metrics = []
...
def generate_metrics_for_selected_pointclouds_fwf_pred(selected_pointclouds, filtered_fwf_pointclouds, metrics_dir, capsel, growsel, output_log):
...
def generate_metrics_for_selected_pointclouds_pred(selected_pointclouds, metrics_dir, capsel, growsel, output_log):
    savename = f"prediction_generated_metrics_{capsel}_{growsel}.csv"
    metrics_path = main_utils.join_paths(metrics_dir, savename)
    if workspace_setup.get_are_fwf_pcs_extracted(metrics_dir) == False:
        all_metrics = []
...
def generate_training_data(output_log, capsel, growsel, filtered_pointclouds, resampled_pointclouds, combined_metrics, images_frontal, images_sideways, sss_testsize, metrics_dir, rfe_threshold):
    ...
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - STATUS - One-Hot encoding labels!" +'\n')
    output_log.configure(state="disabled")
    ...
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - STATUS - Performing stratified shuffle split!" +'\n')
    output_log.configure(state="disabled")
