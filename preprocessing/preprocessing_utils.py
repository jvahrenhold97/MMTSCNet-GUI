...
def traverse_user_specified_data(pathlist, cap_sel, grow_sel, output_log, fwf_av):
    if fwf_av == True:
        config_file_path = pathlist[9]
        pc_path = pathlist[5]
        fwf_path = pathlist[6]
    else:
        config_file_path = pathlist[6]
        pc_path = pathlist[3]
    selection_file_path = workspace_setup_utils.join_paths(config_file_path, "DATA_" + cap_sel + "_" + grow_sel)
    if not os.path.isdir(selection_file_path):
        os.makedirs(selection_file_path)
        if fwf_av == True:
            pc_path_selection = workspace_setup_utils.join_paths(selection_file_path, "las")
            fwf_path_selection = workspace_setup_utils.join_paths(selection_file_path, "fwf")
            img_path_selection = workspace_setup_utils.join_paths(selection_file_path, "img")
            metrics_path_selection = workspace_setup_utils.join_paths(selection_file_path, "metrics")
            os.makedirs(pc_path_selection)
            os.makedirs(fwf_path_selection)
            os.makedirs(img_path_selection)
            os.makedirs(metrics_path_selection)
            ...
            for filepath in selected_pointclouds:
                filename = filepath.split("/")[-1]
                destination = workspace_setup_utils.join_paths(pc_path_selection, filename)
                shutil.copy2(filepath, destination)
                now = datetime.datetime.now()
                now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
                output_log.configure(state="normal")
                output_log.insert("1.0", f"{now_formatted} - INFO - Copied file {filename}!" +'\n')
                output_log.configure(state="disabled")
            for filepath_fwf in selected_fwf_pointclouds:
                filename_fwf = filepath_fwf.split("/")[-1]
                destination_fwf = workspace_setup_utils.join_paths(fwf_path_selection, filename_fwf)
                shutil.copy2(filepath, destination_fwf)
                now = datetime.datetime.now()
                now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
                output_log.configure(state="normal")
                output_log.insert("1.0", f"{now_formatted} - INFO - Copied file {filename_fwf}!" +'\n')
                output_log.configure(state="disabled")
        else:
            pc_path_selection = workspace_setup_utils.join_paths(selection_file_path, "las")
            img_path_selection = workspace_setup_utils.join_paths(selection_file_path, "img")
        ...
        if fwf_av == True:
            ...
            img_path_selection = workspace_setup_utils.join_paths(selection_file_path, "img")
            metrics_path_selection = workspace_setup_utils.join_paths(selection_file_path, "metrics")
            selected_pointclouds = select_data_according_to_specifications(cap_sel, grow_sel, pc_path_selection)
            selected_fwf_pointclouds = select_data_according_to_specifications(cap_sel, grow_sel, fwf_path_selection)
            now = datetime.datetime.now()
            now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
            output_log.configure(state="normal")
            output_log.insert("1.0", f"{now_formatted} - INFO - Files matching configuration found, loading!" +'\n')
            output_log.configure(state="disabled")
        else:
            pc_path_selection = workspace_setup_utils.join_paths(selection_file_path, "las")
            img_path_selection = workspace_setup_utils.join_paths(selection_file_path, "img")
            ...
            output_log.configure(state="disabled")
    if fwf_av == True:
        return selected_pointclouds, selected_fwf_pointclouds, pc_path_selection, fwf_path_selection, img_path_selection, metrics_path_selection
    else:
        return selected_pointclouds, pc_path_selection, img_path_selection, metrics_path_selection
            ...
            if grosel == "ALL":
                if "ALS" in file or "TLS" in file or "ULS" in file:
                    if "LEAF-ON" in file or "LEAF-OFF" in file:
                        filepath = workspace_setup_utils.join_paths(search_directory, file)
...
def augment_selection_fwf(pointclouds, fwf_pointclouds, elimination_percentage, max_pc_scale, output_log):
    if check_if_data_is_augmented_already(pointclouds) == False:
        maximum_scale = max_pc_scale
        ...
        species_pc_pairs = get_species_dependent_pointcloud_pairs_fwf(species_to_use, pointclouds, fwf_pointclouds)
        max_representation = get_maximum_distribution(species_distribution)
        augment_species_pointclouds_fwf(species_pc_pairs, max_representation, species_distribution, maximum_scale, output_log)
    else:
        now = datetime.datetime.now()
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - INFO - Augmented data found, loading!" +'\n')
        output_log.configure(state="disabled")

def augment_selection(pointclouds, elimination_percentage, max_pc_scale, output_log):
    if check_if_data_is_augmented_already(pointclouds) == False:
        maximum_scale = max_pc_scale
        species_list = get_species_distribution(pointclouds)
        species_to_use, species_distribution = eliminate_underrepresented_species(species_list, elimination_percentage)
        species_pointclouds = get_species_dependent_pointclouds(species_to_use, pointclouds)
        max_representation = get_maximum_distribution(species_distribution)
        augment_species_pointclouds(species_pointclouds, max_representation, species_distribution, maximum_scale, output_log)
    else:
        now = datetime.datetime.now()
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
...
def get_species_distribution_fwf(selected_pointclouds, selected_fwf_pointclouds):
    ...
    for pointcloud in selected_pointclouds:
        ...
        for fwf_pointcloud in selected_fwf_pointclouds:
            fwf_filename = fwf_pointcloud.split("/")[-1]
...
def get_species_dependent_pointcloud_pairs_fwf(species_to_use, selected_pointclouds, selected_fwf_pointclouds):
    species_pairs = []
    for pointcloud in selected_pointclouds:
        filename = pointcloud.split("/")[-1]
        tree_id = filename.split("_")[0]
...
def get_species_dependent_pointclouds(species_to_use, selected_pointclouds):
    species_clouds = []
    for pointcloud in selected_pointclouds:
        filename = pointcloud.split("/")[-1]
        species = filename.split("_")[2]
...
def augment_species_pointclouds_fwf(species_pc_pairs, max_representation, species_distribution, max_scale, output_log):
    pair_index = 0
    for species_pairs in species_pc_pairs:
        ...
        for pc_pair in species_pairs:
            filename_reg_full = pc_pair[0].split("/")[-1]
            ...
            filepath_reg = os.path.dirname(pc_pair[0])
            filename_fwf_full = pc_pair[1].split("/")[-1]
            ...
            filepath_fwf = os.path.dirname(pc_pair[1])
            reg_points, reg_pc = load_point_cloud_and_file(pc_pair[0], output_log)
            fwf_points, fwf_pc = load_point_cloud_and_file(pc_pair[1], output_log)
            for i in range(0, int(upscale_fac)*3):
                pair_index+=1
                outFile_r = lp.create(point_format=reg_pc.point_format, file_version="1.4")
                outFile_f = lp.create(point_format=fwf_pc.point_format, file_version="1.4")
                ...
                now = datetime.datetime.now()
                now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
                new_filename_reg = filename_reg + "_" + "aug0" + str(pair_index) + str(i) + "." + filename_reg_ext
                new_filename_fwf = filename_fwf + "_" + "aug0" + str(pair_index) + str(i) + "." + filename_fwf_ext
                output_log.configure(state="normal")
                output_log.insert("1.0", f"{now_formatted} - INFO - Created a point clouds {new_filename_reg} and {new_filename_fwf}!" +'\n')
                output_log.configure(state="disabled")
                savepath_reg = os.path.join(filepath_reg + "/" + new_filename_reg)
                savepath_fwf = os.path.join(filepath_fwf + "/" + new_filename_fwf)
                save_point_cloud(savepath_reg, reg_pc, outFile_r)
                save_point_cloud(savepath_fwf, fwf_pc, outFile_f)

def augment_species_pointclouds(species_pcs, max_representation, species_distribution, max_scale, output_log):
    pc_index = 0
    for pointcloud in species_pcs:
        ...
        pc_name_full = pointcloud.split("/")[-1]
        ...
        pc_points, pc = load_point_cloud_and_file(pointcloud, output_log)
        for i in range(0, int(upscale_fac)*3):
            pc_index+=1
            outFile_r = lp.create(point_format=pc.point_format, file_version="1.4")
            angle = pick_random_angle()
            ...
            outFile_r.x = scaled_rotated_pc[:, 0]
            outFile_r.y = scaled_rotated_pc[:, 1]
            outFile_r.z = scaled_rotated_pc[:, 2]
            new_filename_pc = filename_pc + "_" + "aug0" + str(pc_index) + str(i) + "." + pc_name_extension
            now = datetime.datetime.now()
            now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
            output_log.configure(state="normal")
            output_log.insert("1.0", f"{now_formatted} - INFO - Created point cloud {new_filename_pc}!" +'\n')
            output_log.configure(state="disabled")
            savepath_pc = os.path.join(pc_filepath + "/" + new_filename_pc)
            save_point_cloud(savepath_pc, pc, outFile_r)

def get_species_for_pairs_list(species_pairs):
    first_spec_pair = species_pairs[0]
    reg_pc = first_spec_pair[0]
    filename = reg_pc.split("/")[-1]
    species = filename.split("_")[2]
    return species

def get_species_for_pointcloud(species_pc):
    filename = species_pc.split("/")[-1]
    species = filename.split("_")[2]
    return species
...
def load_point_cloud_and_file(file_path, output_log):
    try:
        ...
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - ERROR - Error loading file {file_path}: {e}!" +'\n')
        output_log.configure(state="disabled")
        raise
    return points, las_file

def load_point_cloud_file(file_path, output_log):
    try:
        las_file = lp.read(file_path)
    except OSError as e:
        now = datetime.datetime.now()
...
def load_point_cloud(file_path, output_log):
    try:
        las_file = lp.read(file_path)
        points = np.vstack((las_file.x, las_file.y, las_file.z)).transpose()
    except OSError as e:
...
def generate_colored_images(IMG_SIZE, las_working_folder, img_working_folder, output_log):
    if get_colored_images_generated(las_working_folder, img_working_folder, output_log) == False:
        pcid = 0
        for pointcloud in os.listdir(las_working_folder):
            pointcloud_path = workspace_setup_utils.join_paths(las_working_folder, pointcloud)
            ...
            now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
            output_log.configure(state="normal")
            output_log.insert("1.0", f"{now_formatted} - INFO - Generated frontal and sideways images for pointcloud {pcid}!" +'\n')
            output_log.configure(state="disabled")
    else:
        pass

def get_colored_images_generated(las_working_folder, img_working_folder, output_log):
    las_list = []
    img_list = []
    for pointcloud in os.listdir(las_working_folder):
    ...
    if imlistlength == len(las_list) or imlistlength > 0:
        ...
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - INFO - Images have already been generated, loading!" +'\n')
        output_log.configure(state="disabled")
        return True
    else:
        return False
...
def resample_point_cloud_density_based(points, num_points, output_log, num_segments=16):
    now = datetime.datetime.now()
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - INFO - Resampling a pointcloud!" + '\n')
    output_log.configure(state="disabled")
...
def generate_metrics_for_selected_pointclouds_fwf(selected_pointclouds, filtered_fwf_pointclouds, metrics_dir, output_log):
    metrics_path = workspace_setup_utils.join_paths(metrics_dir, "generated_metrics.csv")
    if workspace_setup_utils.get_are_fwf_pcs_extracted(metrics_dir) == False:
        all_metrics = []
        now = datetime.datetime.now()
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - INFO - Computing metrics for {len(selected_pointclouds)} pointclouds!" +'\n')
        output_log.configure(state="disabled")
        for i in range(len(selected_pointclouds)):
            las_points = load_point_cloud(selected_pointclouds[i], output_log)
            ...
            now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
            output_log.configure(state="normal")
            output_log.insert("1.0", f"{now_formatted} - INFO - Computed {i+1} of {len(selected_pointclouds)} pointclouds!" +'\n')
            output_log.configure(state="disabled")
        ...
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
        output_log.insert("1.0", f"{now_formatted} - INFO - Previously generated metrics found, importing!" +'\n')
        output_log.configure(state="disabled")
        combined_metrics = load_metrics_from_path(metrics_path)
    return combined_metrics

def generate_metrics_for_selected_pointclouds(selected_pointclouds, metrics_dir, output_log):
    metrics_path = workspace_setup_utils.join_paths(metrics_dir, "generated_metrics.csv")
    if workspace_setup_utils.get_are_fwf_pcs_extracted(metrics_dir) == False:
        all_metrics = []
        now = datetime.datetime.now()
...
def generate_training_data(filtered_pointclouds, resampled_pointclouds, combined_metrics, images_frontal, images_sideways, sss_testsize, metrics_dir, rfe_threshold, output_log):
    ...
    if len(rfe_metrics) > 0:
        ...
        combined_eliminated_metrics = perform_recursive_feature_elimination_with_threshold(combined_metrics, elimination_labels, metrics_dir, output_log, rfe_threshold)
        now = datetime.datetime.now()
        now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
        output_log.configure(state="normal")
    ...
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - INFO - Tree species to train on: {np.unique(tree_labels)}" +'\n')
    output_log.configure(state="disabled")
    now = datetime.datetime.now()
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    ...
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - INFO - Performing Stratified Shuffle Split!" +'\n')
    output_log.configure(state="disabled")
...
def perform_recursive_feature_elimination_with_threshold(X, y, metrics_dir, output_log, importance_threshold):
    """
    ...
    now_formatted = now.strftime("%Y-%m-%d %H:%M:%S")
    output_log.configure(state="normal")
    output_log.insert("1.0", f"{now_formatted} - INFO - Performing Recursive Feature Elimination!" +'\n')
    output_log.configure(state="disabled")
    ...
    metrics_path = workspace_setup_utils.join_paths(metrics_dir, "rfe_generated_metrics.csv")
